...
Unix shells are command line interpreters for [Unix-like operating systems (OS)](https://en.wikipedia.org/wiki/Unix-like)[^1].

In contrast to [graphical user interfaces (GUIs)](https://en.wikipedia.org/wiki/Graphical_user_interface), the user enters commands as text—interactively in a terminal or via scripts (a text file with a number of commands)—and the shell passes them to the OS.
...
Knowing how to interact with a machine via shells allows you to use computers that do not have a [windowing system](https://en.wikipedia.org/wiki/Windowing_system) installed or to use them very efficiently without launching the windowing system.
