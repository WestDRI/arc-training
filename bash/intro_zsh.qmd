...
You will probably want to add to this file the aliases, functions, and sourcing that you normally have in your `.bashrc` file. For instance, in the [modern utilities](intro_modern.qmd) section, we added the following to our `.bashrc` file:

```{.bash}
[[ -s $EPREFIX/etc/profile.d/autojump.sh ]] && source $EPREFIX/etc/profile.d/autojump.sh
```

If you want to use `autojump` in Zsh, you should add this to your `.zshrc` file.
