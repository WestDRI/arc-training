...
*Content from [the webinar slides](wb_dvc_slides.qmd) for easier browsing.*
...
![On the benefits of VCS](/git/img/vc.jpg){width="45%"}
...
bgcolor="transparent"
node [fontname="Inconsolata, sans-serif", fontsize=9, penwidth=0.5, fontcolor=gray55]
...
### Visualize pipeline in a [DAG](https://en.wikipedia.org/wiki/Directed_acyclic_graph)
...
- When the 2^nd^ and 3^rd^ stages are run, `dvc.lock` is updated. At the end of the run `dvc.lock` contains all the info about the run we just did (version of the data used, etc.).
...
- [DVCLive](https://github.com/iterative/dvclive) (Python library to log experiment metrics).
- Visualize the performance logs as [plots](https://dvc.org/doc/user-guide/experiment-management/visualizing-plots).
- Continuous integration with [CML](https://github.com/iterative/cml) (Continuous Machine Learning).
