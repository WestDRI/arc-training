...
It appears that it is also now possible to log in via your browser (a pop-up will open when you push for you to sign in).
...
- When prompted to `Enter the path of a file in which to save the key`, you can press `Enter` to accept the default file path. If you have already created SSH key pairs, `ssh-keygen` may ask to overwrite another pair. In that case, give your new key a custom name.
...
The first time you will connect to GitHub via SSH (the first time you will push to an SSH address), you will be asked:
