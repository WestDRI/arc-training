...
Now, let's build an alias for a more complex command: `git grep "test" $(git rev-list --all)`. This an example
from the *"Searching a Git project"* section below, and will search for the string "test" in all earlier
...
It turns out you can still automate this with a Git alias! You need to place a executable bash script into a
directory that is listed in your `$PATH` environment variable. For example, let's assume that `$HOME/bin` is
listed in your `$PATH`. Doing the following
