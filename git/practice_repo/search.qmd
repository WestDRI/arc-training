...
If you don't want to have to type a series of flags all the time, you can configure [aliases for Git](https://git-scm.com/book/en/v2/Git-Basics-Git-Aliases). For instance, Alex Razoumov uses the alias `git search` for `git grep --break --heading -n -i`.
