---
title: High-performance computing in ![](img/logo_r.png){width="2em"}
author: Marie-Hélène Burle
---

:::{.def}

*Content from [the webinar slides](wb_hpc_slides.qmd) for easier browsing.*
...
To compile R packages, you need a C compiler. A module for `gcc` (the GNU project C and C++ compiler—part of the [the GNU Compiler Collection (GCC)](https://gcc.gnu.org/)) is already loaded by default, so you don't have anything to do. You can double-check that a `gcc` module is loaded by running `module list`.
...
`StdEnv/2023` is a required module for this version, but on most Alliance clusters, it is automatically loaded, so you don't need to include it. You can double-check with `module list` or you can include it (before `r/4.5.0`) just to be sure.

:::

Finally, load your module:
...
module load StdEnv/2020 gcc/11.3.0 r/4.5.0
