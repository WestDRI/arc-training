...
If you want to create multiple plots in the same frame, you simply add them to each other:
...
plot2 <- ggplot(mpg, aes(x = displ, y = hwy)) +
  geom_smooth(se = FALSE, color = "#999999", linewidth = 0.5) +
  labs(
    x = "Engine size (L)",
    y = "Fuel economy (mpg) on highways"
  ) +
...
More complex layouts can easily be built with the [`patchwork`](https://patchwork.data-imaginist.com/) package.

## `ggplot2` extensions

Thanks to its vast popularity, `ggplot2` has seen [a proliferation of packages extending its capabilities](https://exts.ggplot2.tidyverse.org/gallery/):
