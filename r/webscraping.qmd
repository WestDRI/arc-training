...
Again, before running a for loops, we need to initialize empty lists:

```{.r}
list_data <- vector("list", length(list_links))
html <- vector("list", length(list_links))
date <- vector("list", length(list_links))
major <- vector("list", length(list_links))
pi <- vector("list", length(list_links))
```
...
for (i in seq_along(list_links)) {
  ...
  pi[[i]] <- html[[i]] %>%
    ...
    html_text2()
  list_data[[i]] <- cbind(date[[i]], major[[i]], pi[[i]])
}
```
...
All looking good, so let's turn this big list into a tibble (a [tidyverse](https://www.tidyverse.org/) dataframe):
