...
We will explore an example to calculate the sum of 1e4 random vectors of length 3.
...
```{r}
set.seed(2)
result2 <- foreach(i = 1:1e4, .combine = '+') %do% runif(3)
```
...
```{.r}
set.seed(2)
result3 <- foreach(i = 1:1e4, .options.future = list(seed = TRUE), .combine = '+') %dofuture% {
  runif(3)
}
```
...
```{.r}
set.seed(2)
result3 <- foreach(i = 1:1e4, .combine = '+') %dofuture% {
  runif(3)
} %seed% TRUE
```
...
```{r}
set.seed(2)
result2 <- foreach(i = 1:1e4, .combine = '+') %do% {
  Sys.sleep(0.01)
  runif(3)
}
```
...
First, let's time the classic while loop:
