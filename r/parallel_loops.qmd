...
The best part of `foreach` is that you can turn a sequential loop into a parallel one if you register a parallel backend, then replace `%do%` with `%dopar%`.
...
Because we haven't registered any parallel backend, the expression will still be evaluated sequentially. To run this in parallel, we need to load `doFuture`, register it as a backend (with `registerDoFuture()`), and choose a parallel strategy (e.g. `plan(multicore)`):
...
Because of the overhead of parallelization, it actually doesn't make sense to parallelize such a short code, but let's go over a toy example and do some benchmarking.
