...
/var/spool/slurmd/job00021/slurm_script: line 5: 133216 Killed                  Rscript bigger.R
slurmstepd: error: Detected 1 oom_kill event in StepId=21.batch. Some of the step tasks have been OOM Killed.
```
...
Similarly, using `foreach` and `doFuture` as we did previously won't help either because that spreads the number of trees on various cores, but again, the problem doesn't come from the number of trees, but from the size of the dataset.
...
You could split the data manually and run the code on each chunk, but it would be tedious and very lengthy. You could also implement running the code on all chunks in parallel, but there is a much simpler option provided by the [multidplyr](https://cran.r-project.org/web/packages/multidplyr/index.html) package.
...
```{.bash}
salloc --time=50 --mem-per-cpu=3600M --cpus-per-task=4
```
...
# You can check that the rows are shuffled
...
Now we need to bring the partitioned results to the main process:
...
Finally, we combine the 4 randomForest models into a single one. This can be done with the `combine()` function from the `randomForest` package (the same function we already used in our foreach expressions):
...
This is it: by splitting our data frame on 4 cores, we could run the code and create a randomForest model using the whole dataset.
...
We want to make sure to compare the same things. So we go back to our smaller `big_iris` and we increase the number of trees back to 1000.

We will compare it with the plans `multisession` and `multicore` that [we performed earlier](hpc_foreach#multisession.qmd). The minimum and median times for these two options for shared memory parallelism were of 2.72s and 3.15s respectively.
...
part_rf <- function(data, cluster) {
  ...
  rfs <- split_data %>%
    do(rf = randomForest(Species ~ ., data = ., ntree = 1000)) %>%
    collect()
...
In cases where your data is so big that it doesn't fit in the memory of a single node, it doesn't seem that any R package currently allows to split the data automatically for you.

:::

### Conclusion

As we could see, we got similar results: in this case, it is the same to spread the number of trees running on the full data on 4 cores or to run all the trees on the data spread across 4 cores.

One difference however is that `foreach` and `doFuture` allowed us to spread the trees across nodes while `multidplyr` does not allow this for the data.
