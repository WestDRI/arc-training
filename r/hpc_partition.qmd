...
Then we create a partitioned data frame on the workers:
...
The partitioning is done automatically, trying to split the data frame as evenly as possible among workers.
...
Finally, we need to combine the 4 randomForest models into a single one. This can be done with the `combine()` function from the `randomForest` package:
...
Here, I want to explore two questions:

- Can multidplyr run in distributed memory? There is nothing on this in the documentation, so let's try it.
- Beside the memory advantage, how is the speedup of this data parallelization? i.e. how does it compare with the parallelization we did as regard the number of trees with `foreach` and `doFuture`? Let's try that too.
...
mark(rf_all <- part_rf(bigger_iris, cl))
```
...
1 rf_all <- pa…  5.5s   5.5s     0.182        NA    0.545     1     3       5.5s
# ℹ 4 more variables: result <list>, memory <list>, time <list>, gc <list>
Warning message:
Some expressions had a GC in every iteration; so filtering is disabled.
```

```{.r filename="partition_dis.R"}
library(multidplyr)
library(dplyr, warn.conflicts = FALSE)
library(bench)
...
bigger_iris <- iris[rep(seq_len(nrow(iris)), each = 1e2), ]
rownames(bigger_iris) <- NULL
...
Comparison with foreach and doFuture:

```{.r filename="partition2_shared.R"}
library(multidplyr)
library(dplyr, warn.conflicts = FALSE)
library(bench)
...
Since we determined that `multidplyr` does not allow for distributed memory, we will compare it with the plans `multisession` and `multicore` that [we performed earlier](https://mint.westdri.ca/r/hpc_foreach#multisession). The minimum and median times for these two options for shared memory parallelism were of 2.72s and 3.15s respectively.
...
As we can see, we are getting very similar results. The difference is that if we up the number of workers to 8 and run it on 2 nodes with 4 cores per node, we aren't getting more speedup because `multidply`'s way of creating for a cluster of workers doesn't have a mechanism to spread them across nodes.
