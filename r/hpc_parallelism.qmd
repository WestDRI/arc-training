...
GNU R was not built with multi-threading capabilities.

:::{.note}

A version of R with multi-threading called R Open was developed by Revolution Analytics, then bought by Microsoft in 2015. The development has now been stopped.

Many R blog posts use the term "multi-threading" improperly and actually mean "multi-processing" (outside the supercomputer HPC world, the terminology around parallel computing is used very loosely).

:::

Proper multi-threading cannot be achieved directly in R. However, some R packages run other languages capable of multi-threading under the hood and bring multi-threading to R. [`brms`](https://cran.r-project.org/web/packages/brms/index.html), which runs [Stan](https://en.wikipedia.org/wiki/Stan_(software)) as a backend, is one such example.
...
The different processes need to communicate with each other, but because they all run on the same node, messages can pass via shared memory.
