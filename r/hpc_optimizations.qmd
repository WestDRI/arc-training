...
If you haven't done so yet, load the R module:

```{.bash}
module load r/4.5.0
```

Now launch an interactive job on one CPU:

```{.bash}
salloc --time=2:00:00 --mem-per-cpu=3600M
```

:::{.note}

Run this in the Bash shell, not in R.
...
Finally, load the benchmarking package that we will use throughout this section:
...
It is ok to run an interactive job here because we are actively developing code and using very little resources. Remember that the way to run your code at scale while using resources optimally is to run a batch job instead. To do this you would:

- Create an R script (called for instance `optim.R`) with the code to run. Don't forget to load the package `bench` in your script.
- Create a bash script (called for instance `optim.sh`) with the following:
...
You can try this with any of the code snippets below as an exercise.
...
`f2()` is consistently faster, although not in any meaningful way in this example. In many cases, the difference you will find will be a lot greater.
...
The output you can see on this site was obtained on my laptop. [It shows that a properly written function with pre-allocated memory uses 14 times less memory.]{.emph}
