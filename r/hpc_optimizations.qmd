...
`f2()` is consistently faster, although not by much (speedup of 1.3). In many cases, the difference you will find will be a lot greater.
...
Now, we compare the timing with that of `f2()`:

```{r}
mark(f2(n), sapply(1:n, f3))
```

As you can see, the loop is faster (speed up of 1.5). On my laptop, it also used 3 times less memory.
...
Now, let's benchmark it against `f2()`:
...
1 f2(n)         116ms  120ms      7.45        NA    26.1      4    14      537ms
2 f4(n)         925ms  925ms      1.08        NA     3.24     1     3      925ms
# ℹ 4 more variables: result <list>, memory <list>, time <list>, gc <list>
Warning message:
Some expressions had a GC in every iteration; so filtering is disabled.
```

`f4()` is 7.7 times slower.
...
Benchmark against `f2()`:

```{.r}
mark(f2(n), f5(n), check = F)
```
...
# A tibble: 2 × 13
  expression      min median `itr/sec` mem_alloc `gc/sec` n_itr  n_gc total_time
  <bch:expr> <bch:tm> <bch:>     <dbl> <bch:byt>    <dbl> <int> <dbl>   <bch:tm>
1 f2(n)         105ms  108ms     9.10         NA    30.9      5    17      549ms
2 f5(n)            6s     6s     0.167        NA     3.34     1    20         6s
# ℹ 4 more variables: result <list>, memory <list>, time <list>, gc <list>
Warning message:
Some expressions had a GC in every iteration; so filtering is disabled.
```
...
Now we only have two modulo operations and equality tests per iteration and we get another little speedup when we benchmark it against our best function:
...
If we used a silly function such as `f5()` as our starting function, the speedup would be 333.
