...
A module for `gcc` (the GNU project C and C++ compiler—part of the [the GNU Compiler Collection (GCC)](https://gcc.gnu.org/)) is already loaded by default, so you don't have anything to do. You can double-check that a `gcc` module is loaded by running `module list`.
...
Whether you are in Bash, Zsh, the Julia REPL, the Python shell, or [the R Console]{.emph}, you can use the same set of keybindings which come from the text editor [Emacs](../emacs/top_intro.qmd).
...
It uses a small application called a pager (usually [less](https://en.wikipedia.org/wiki/Less_(Unix))). Below are keybindings that will help you navigate the pager (and probably most importantly close it!).
...
Example to launch an interactive job on a single CPU with 3600MB of memory for 2h:
...
Example to run a script on 4 CPUs with 3600MB per CPU for 15min:
