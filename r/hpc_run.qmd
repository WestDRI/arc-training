...
In [our introduction to R](top_intro.qmd), we used an RStudio server running on a remote cluster. In this course, we will log in a similar remote supercomputer using [Secure Shell](https://en.wikipedia.org/wiki/Secure_Shell), then run R scripts from the command line.
