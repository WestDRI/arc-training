...
If you just logged back in, you need to load the R module again:

```{.bash}
module load r/4.5.0
```

First, let's launch a short interactive job on one CPU:

```{.bash}
salloc --time=30 --mem-per-cpu=3600M
```
...
The goal is to create a random forest model (let's call it `rf`) that can classify an iris flower as one of the 3 species based on its sepals and petals measurements.
...
Because randomForest relies on random sampling, we need to set a seed if we want to make our code reproducible.
...
The `.` on the right side of the formula represents all other variables (so we are using all other variables as feature variables).
...
And then we can run `randomForest()` on this dataset and 1000 trees.
...
big_iris <- iris[rep(seq_len(nrow(iris)), each = 2e2), ]
rownames(big_iris) <- NULL

set.seed(123)
rf <- randomForest(Species ~ ., data=big_iris, ntree=1000)
```
...
The most useful part of `foreach` is that it allows for easy parallelization with countless backends: `doFuture`, `doMC`, `doMPI`, `doParallel`, `doRedis`, `doRNG`, `doSNOW`, and `doAzureParallel`.

The [doFuture](https://cran.r-project.org/web/packages/doFuture/index.html) package is the most modern of these backends. It allows to evaluate `foreach` expressions across the evaluation strategies of the [future](https://cran.r-project.org/web/packages/future/index.html) package very easily. All you have to do is to register it as a backend, declare the evaluation strategy of your choice, make sure to generate parallel-safe random numbers for reproducibility (if your code uses randomness), and replace `%do%` with `%dofuture%`.
...
set.seed(123)
mark(rf <- randomForest(Species ~ ., data=big_iris, ntree=1000))
```
...
On the other hand, it is more memory efficient: if you increase the number of trees to 2000 and try to run the traditional code, you will get an out of memory error and the job will be killed, while the foreach expression runs fine with the same amount of memory.
...
The latter is better because it allows us to use 2 full nodes instead of having tasks running on any number of nodes (and more nodes might lead to more overhead due to interconnect communication). On the other hand, this also means that we might have to wait longer for the job to run as it is more restrictive (it will only start when 2 full nodes are available).
