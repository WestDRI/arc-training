...
There are many excellent and popular deep learning frameworks already, so why did Google—already behind the successful [TensorFlow](https://www.tensorflow.org/) project—start developing [JAX](https://jax.readthedocs.io/en/latest/index.html)?

JAX is still relatively new and not widely adopted yet, so why teach it instead of [PyTorch](https://pytorch.org/) or TensorFlow?

In this section, we will look at the rationale behind the creation of a new tool and what makes JAX an important project.
...
## The need for new deep learning frameworks

[Automatic differentiation (autodiff, or AD)](https://en.wikipedia.org/wiki/Automatic_differentiation) is the evaluation par computer programs of the partial derivatives of functions. It is a key part of deep learning since training a model consists of updating its weights and biases to decrease some loss function thanks to various gradient-based optimizations.
...
Removing the [Julia](https://julialang.org/) stuff (which is very interesting in itself: Julia has a lot to offer in the field of AD and deep learning) and PyTorch's attempt at JIT compilation, [Chris Rackauckas](https://chrisrackauckas.com/)'s post can be summarized this way:
...
TensorFlow's initial approach with computational graphs in a domain-specific language was unintuitive, inconvenient, and hard to debug. PyTorch came with a xxxx, so much more convenient that it marked the beginning of the decline of TensorFlow. However xxxx. TensorFlow2 tried xxxx, but xxx.

This leaves the room for new strategies. As mentioned above, Julia offers several promising approaches, but implementations are not straightforward and projects are not very mature. It is an exciting avenue for developers, but not necessarily easy for end users. [JAX](https://jax.readthedocs.io/en/latest/index.html) is another attempt at bringing new xxx to the problem. With Google behind it, it is a new but fast growing project.
