...
Function side-effects are not recorded by the tracers and are thus absent from the Jaxpr. This means that they will be executed once (during tracing) and are thereafter absent from the JAX architecture. [For this reason, only pure functions (functions without side effects) should be used with JAX.]{.emph}

A number of composable transformations can be applied to the Jaxpr, the last of which being JIT compilation.
...
The tracer objects used to create the Jaxpr contain information about the shape and dtype of the initial Python arguments, but not their values. This means that the intermediate representation can then be rerun directly with new values, skipping the (slow) Python code.
