---
title: AI-powered coding with
frontpic: img/copilot.png
frontpicwidth: 70%
noshadow: noshadow
author: Marie-Hélène Burle
date: 2024-01-23
date-format: long
execute:
  error: true
  echo: true
format:
  revealjs:
    # embed-resources: true
    theme: [default, ../revealjs.scss]
    logo: /img/logo_sfudrac.png
    highlight-style: ayu
    code-line-numbers: false
    template-partials:
      - ../title-slide.html
    pointer:
      color: "#b5111b"
      pointerSize: 32
    link-external-newwindow: true
    footer: <a href="wb_copilot.html"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="rgb(153, 153, 153)" class="bi bi-arrow-90deg-up" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M4.854 1.146a.5.5 0 0 0-.708 0l-4 4a.5.5 0 1 0 .708.708L4 2.707V12.5A2.5 2.5 0 0 0 6.5 15h8a.5.5 0 0 0 0-1h-8A1.5 1.5 0 0 1 5 12.5V2.707l3.146 3.147a.5.5 0 1 0 .708-.708l-4-4z"/></svg>&nbsp;Back to webinar page</a>
    auto-stretch: false
revealjs-plugins:
  - pointer
---

# Code generators

## State of the field {.center}

# GitHub Copilot

## What is [GitHub Copilot](https://github.com/features/copilot/)? {.center}

:::{.fragment}

**→ Cloud-hosted AI programming assistant**

:::

:::{.fragment}

Developed by [GitHub](https://github.com) (Microsoft)

Running [Codex](https://openai.com/blog/openai-codex), a model by [OpenAI](https://openai.com/) derived from GPT-3 and trained on open source code

:::

## Access {.center}

Individual or organization GitHub accounts

Requires subscription

Students, teachers, and maintainers of popular open source projects can apply for free access

## Safety {.center}

Generated code comes with no guaranty of safety or quality

Filters in place for offensive words

## Supported languages {.center}

Any language used in public repos

Quality of suggestions is higher for languages with lots of data

## How to use it? {.center}

:::{.fragment}

Start typing code and get autocomplete suggestions

&emsp;&emsp;→ for lines \
&emsp;&emsp;→ for entire functions

:::

:::{.fragment}

Write comments describing what the code should do and get code generation based on context

:::

## Interface {.center}

:::{.fragment}

As an extension to text editors \
→ Visual Studio Code/Visual Studio, Vim/Neovim/Emacs, JetBrains IDEs, Azure Data Studio

:::

:::{.fragment}

In the command line

:::

# Setup

## Get a subscription {.center}

Got to your GitHub account page \
&emsp;→ Settings \
&emsp;→ Copilot \
&emsp;→ Enable

Provide free access or payment method

Set settings

## VS Code {.center}

[Install the GitHub Copilot extension](https://marketplace.visualstudio.com/items?itemName=GitHub.copilot)

## Vim/Neovim {.center}

Install [Node.js](https://nodejs.org/en/download/)

Clone <https://github.com/github/copilot.vim>

Configure:

```{.bash}
:Copilot setup
```

Enable:

```{.bash}
:Copilot enable
```

Get help:

```{.bash}
:help copilot
```

## Emacs {.center}

# Usage

## VS Code {.center}

Next suggestion: `Alt+]` \
Previous suggestion: `Alt+[`

Reject suggestion: `Esc` \
Accept suggestion: `Tab` \
Accept next suggested word: `Ctrl+→` \
(`Command+→` &nbsp;for macOS)

Open new tab with options: `Ctrl+Enter`

Or hover over suggestion

Set your own key binding for `editor.action.inlineSuggest.acceptNextLine` to accept next suggested line

## Emacs {.center}

# Copilot in the CLI

## What is Copilot in the CLI? {.center}

*In beta*

An extension to [GitHub CLI](https://cli.github.com/) (GitHub operations from the CLI)

→ Generate commands from natural language \
→ Generate natural language explanations from commands

Trained on data up to 2021

Lower performance for natural languages ≠ English

## Setup {.center}

:::{.fragment}

[Install GitHub CLI](https://github.com/cli/cli#installation)

Connect to your GitHub account:

```{.bash}
gh auth login
```

:::

:::{.fragment}

<br>
Install Copilot in the CLI:

```{.bash}
gh extension install github/gh-copilot
```

:::{.note}

Update with: `gh extension upgrade gh-copilot`

:::

:::

## Usage {.center}

Get code explanations:

```{.bash}
gh copilot explain
```

<br>
Get code from natural language:

```{.bash}
gh copilot suggest
```

## Example of code suggestion {.center}

```{.bash}
gh copilot suggest
```

```
Welcome to GitHub Copilot in the CLI!
version 0.5.4-beta (2024-01-04)

I'm powered by AI, so surprises and mistakes are possible. Make sure to verify any generated code or suggestions, and share feedback so that we can learn and improve. For more information, see https://gh.io/gh-copilot-transparency

? What kind of command can I help you with?  [Use arrows to move, type to filter]
> generic shell command
  gh command
  git command
```

## Code suggestion {.center}

```
? What would you like the shell command to do?
> rename all files matching regexp recursively


Suggestion:
```

```{.bash}
find . -type f -name '*regexp*' -exec rename -v 's/regexp/newname/g' {} +
```

```
? Select an option  [Use arrows to move, type to filter]
  Copy command to clipboard
> Explain command
  Revise command
  Rate response
  Exit
```

## Code suggestion {.center}

```
Explanation:

  • find . is used to recursively search for files starting from the current directory.
    • -type f specifies that we want to search for regular files.
    • -name '*regexp*' specifies that we want to search for files matching the pattern *regexp*.
    • -exec rename -v 's/regexp/newname/g' {} + executes the rename command for each found file.
      • rename is a command for renaming files.
      • -v prints verbose output while renaming.
      • 's/regexp/newname/g' is a Perl-style regular expression replacement pattern.
        • regexp is the pattern to be matched.
        • newname is the replacement string.
        • g flag means that the replacement should be applied globally (for all matches).
      • {} is a placeholder that represents the found filename.
      • + signifies the end of the -exec command and that multiple files can be passed at once.
```

## Getting help {.center}

[GitHub support portal](https://support.github.com/)

[Stack Overflow [github-copilot] tag](https://stackoverflow.com/questions/tagged/github-copilot)
