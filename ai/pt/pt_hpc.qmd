...
- Only request the time that you will really use (e.g. for [the lesson on PyTorch tensors](pt_tensors.qmd), maybe 30 min to 1 hour seems reasonable)
...
![from [the Alliance Wiki](https://docs.alliancecan.ca/wiki/Using_GPUs_with_Slurm)](img/cc_gpu.png)
...
It is very likely that you do not need more than one GPU. Asking for more than you need will greatly increase your waiting time until your job is run. The short notes on [distributed computing with PyTorch](https://westgrid-ml.netlify.app/2022_modules/18_distributed/) give a few information as to when you might benefit from using several GPUs and provides some links to more resources. We will also offer workshops on distributed ML in the future. In any event, you should test your model before asking for several GPUs.
