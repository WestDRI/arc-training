...
- [Polars](https://github.com/pola-rs/polars) poorly integrated
- UI can be inconsistent
- Database and servers are not very resilient to by bad workflows, deletion and recreation of files, etc.
- Nested runs don't get logged as children runs if workflow is not followed carefully
...
**Model Uniform Resource Identifier (URI), `model_uri`**: \
Unique sequence of characters that identifies a model artifacts (the model and its associated data)
...
```{.bash}
ssh -N -f -L localhost:5000:<node>:5000 \
	   <user>@<cluster>.alliancecan.ca
```
...
uv run mlflow server \
       --backend-store-uri sqlite:///demos.db
```
...
Let's go back to the objective function we defined in the last demo as an example
...
```{.python code-line-numbers="15-19"}
            "max_features": rf_max_features,
        }
        # Log current trial's parameters
        mlflow.log_params(params)
...
train_dataset = mlflow.data.from_polars(
    train_data, name="calhousing_train"
)

val_dataset = mlflow.data.from_polars(
    val_data, name="calhousing_val"
)
...
You can do so when you log it by adding a value to the `registered_model_name` argument of the `log_model` function, but often, you will do so afterwards, once you have looked at the logs and decided that they look good
