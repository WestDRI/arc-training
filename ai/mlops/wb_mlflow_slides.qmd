...
<br>[MLflow projects](https://mlflow.org/docs/latest/ml/projects/#environment) do not (yet) support [uv](https://docs.astral.sh/uv/)
...
- You log tracking data
- You launch an MLflow server
- You open the user interface in a browser to visualize the logged data
...
^1^Usage without databases will be deprecated at some point
...
*From [MLflow website](https://mlflow.org/docs/latest/ml/getting-started/deep-learning/)*
...
mlflow.set_tracking_uri("sqlite:///model_tracking_demo.db")
mlflow.set_experiment("Model tracking demo")
...
*From [MLflow website](https://mlflow.org/)
...
mlflow.set_tracking_uri("sqlite:///tuning_demo.db")
mlflow.set_experiment("Tuning demo")
```
...
```{.bash}
mlflow server \
       --backend-store-uri sqlite:///tuning_demo.db
```
...
Select the experiment "Model tracking demo"

You can see graphs of model metrics (loss and accuracy) \
as well as system metrics (CPU/GPU/disk/memory usage)
