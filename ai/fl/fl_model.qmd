...
[As we already saw](fl_state#example-pytorch-vs-flax.qmd), Linen provides two syntaxes:
...
Printing the pytree of the initial parameters we created would make it fairly hard to get a sense of their structure, but [we know that JAX can operate on pytrees](/ai/jx/jx_pytree.qmd) thanks to the `tree_util` module, so let's make use of this:
