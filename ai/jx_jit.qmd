...
The problem here is that the shape of the argument to `prod` depends on the value of `x`, but jaxprs do not record values.

One solution is to use the NumPy version of `prod`: it will not be JIT compiled (not such thing with NumPy) and that allows us to still optimize the rest of our function:
