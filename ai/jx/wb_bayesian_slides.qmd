...
It doesn't assign probabilities to non-random variables such as hypotheses or parameters (a hypothesis is either true or false, so we can't speak of the probability of a hypothesis being true. Similarly, we can't speak of the probability of a parameter falling in a confidence interval—either it is in it or it is not)

Instead, the probability is assigned to the limit of the relative frequencies of events in infinite trials (for events for which long-runs don't make sense, one imagines alternative realities and considers the frequency of occurrences in those realities) and we can assign a probability to the fact that a new random sample would produce a confidence interval that contains the unknown parameter

This is not how we intuitively think and the results are hard to interpret. The frequentist approach is also often artificially constrained and limits the integration of various forms of information
