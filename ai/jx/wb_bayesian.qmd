...
With the advent of increasingly performant [probabilistic programming languages (PPLs)](https://en.wikipedia.org/wiki/Probabilistic_programming), algorithms, compilers, [automatic differentiation](https://en.wikipedia.org/wiki/Automatic_differentiation) engines, and computer hardware, Bayesian approaches are now fast growing in popularity.

[JAX](https://docs.jax.dev/en/latest/index.html) is a library for Python that makes use of the extremely performant [XLA compiler](https://en.wikipedia.org/wiki/Accelerated_Linear_Algebra), runs on accelerators (GPUs/TPUs), provides automatic differentiation, [just-in-time compilation](https://en.wikipedia.org/wiki/Just-in-time_compilation), batching, and parallelization. In short, it is a perfect tool for Bayesian statistics. Not surprisingly, many PPLs now use it as a backend and [Stan](https://mc-stan.org/) users (and developers!) are turning to it.
