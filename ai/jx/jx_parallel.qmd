...
[Remember how a number of transformations are applied to jaxprs](/ai/jx_map#map.qmd). We already saw two of JAX's main transformations: JIT compilation with `jax.jit` and automatic differentiation with `jax.grad`. Vectorization with `jax.vmap` is another one.
