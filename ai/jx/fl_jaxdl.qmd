...
- **Train**
  ...
  Training a model requires optimization functions. These can be implemented in JAX from scratch but the library [Optax](https://optax.readthedocs.io/en/latest/) provides the core components. [Orbax](https://github.com/google/orbax) provide checkpointing utilities.
...
🤗 [fontcolor=burlywood3]
flax1 [label=Flax, group=g2, fontcolor="#669900"]
flax2 [label=Flax, fontcolor="#669900"]

{rank=same; load PyTorch TensorFlow 🤗}
load -> PyTorch -> TensorFlow -> 🤗 [style=invis]

{rank=same; nn flax1}
nn -> flax1 [style=invis]

jax1 [label=<<FONT COLOR="deepskyblue3">JAX</FONT><BR/><FONT COLOR="darkorange4">Optax</FONT><BR/><FONT COLOR="darkorange4">Orbax</FONT>>]

jax2 [labe="JAX", fontcolor=deepskyblue3]

{PyTorch TensorFlow 🤗} -> flax1 -> jax1 -> jax2 -> flax2
