...
proc [label="Process data", shape=plaintext, group=g1]
nn [label="Define architecture", shape=plaintext, group=g1]
opt [label="Optimize", shape=plaintext, group=g1]
...
The meta-library [jax-ai-stack](https://github.com/jax-ml/jax-ai-stack) makes this easier to manage

TensorFlow and packages which depend on it (e.g. TFDS, grain) are still stuck at Python 3.12, so you can't use a newer Python version if you want to use some of them

[**On your machine only**]{.emph}, [`uv`](https://github.com/astral-sh/uv) makes managing all this easy ([webinar coming soon](https://mint.westdri.ca/python/wb_uv)). On the clusters, use `module` to load the Python version you want and `pip` to install packages
