...
```{python}
def translator(df, class_id):
    species = df.filter(
        pl.col('class_id') == class_id
    ).select(pl.col('species')).item()

    subcategory = df.filter(
        pl.col('class_id') == class_id
    ).select(pl.col('subcategory')).item()

    return species, subcategory
```

```{python}
print(translator(class_id_to_name, 860))
print(translator(class_id_to_name, 900))
print(translator(class_id_to_name, 400))
```
