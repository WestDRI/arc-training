...
[`grain.SequentialSampler`](https://google-grain.readthedocs.io/en/latest/_autosummary/grain.samplers.SequentialSampler.html)
...
Something else we need to do is split the data into batches. Batches can be defined with `grain.Batch` as a DataLoader transformation.
...
| Feature | **Small Batch Size** (e.g., 16, 32) | **Large Batch Size** (e.g., 256, 1024+) |
| :--- | :--- | :--- |
| **Training Speed** | Slower: less efficient use of GPU parallelism | Faster: maximizes GPU throughput |
...
We now have our source, sampler, and transformation, so we can build our first Grain DataLoader:
...
[`grain.IndexSampler`](https://google-grain.readthedocs.io/en/latest/_autosummary/grain.samplers.IndexSampler.html)
