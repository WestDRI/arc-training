...
An important step if you want to train a model on the Alliance clusters using your own dataset is to transfer the data to the cluster.
...
If you can use `curl` or `wget` to do so, download it directly in the cluster.

Unfortunately, [the data source](https://dl.allaboutbirds.org/nabirds) does not provide a direct access. Instead, you have to log in before ending up in a DropBox. So the only option is to download it to your own machine first. Then you need to transfer it to the cluster.
...
To launch the transfer, you also need to install [Globus Connect Personal](https://www.globus.org/globus-connect-personal) on your computer to create an endpoint for your machine.
...
If neither a direct download nor Globus are an option, there are other methods, although for large datasets, you might run into problems.

:::{.info}

A trick that will make any of the following command involving the path to a remote cluster easier is to create an SSH config file as `~/.ssh/config`.
