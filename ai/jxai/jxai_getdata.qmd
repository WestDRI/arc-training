...
An important step if you want to train a model on the Alliance clusters using your own dataset is to transfer it to the cluster.
...
If you can use `curl` or `wget` to do so, download it directly to the cluster.

Unfortunately, [the data source for our example](https://dl.allaboutbirds.org/nabirds) does not provide a direct access. Instead, you have to log in before ending up in a DropBox. So the only option is to download it to your own machine first. Then you need to transfer it to the cluster.
