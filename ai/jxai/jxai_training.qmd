...
[Orbax](https://github.com/google/orbax) provide checkpointing utilities for JAX.

### Stopping

If the training is very long, how do you know how many epochs you need to choose?

You don't want to take a random guess. You also don't want to spend your days and nights staring at TensorBoard, MLflow or your terminal. So you need to set things up to have the training stop automatically.
...
You can use an if/else statement to automatically stop training.

To be more sophisticated and avoid problems such as [double descent](https://en.wikipedia.org/wiki/Double_descent), instead of stopping as soon as this happens, one hyperparameter that can be set is **patience**: the number of epochs the model should continue training after the validation loss goes up (usually 3 to 5). This allows the model to recover from temporary plateaus.

Once that patience value is reached, you should use the checkpoint for the best validation loss (before the patience period).
