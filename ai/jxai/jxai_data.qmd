...
Now we can combine them together (no need of a Compose class with Grain as with TorchVision):
...
In PyTorch, you would apply the Transforms at this point since the Dataset class has a `transform` argument in which to pass them:
...
for i, sample in enumerate(nabirds_norm_train_pytorch):
...
With Grain, you pass the combined Transforms as an argument of the DataLoader class.
