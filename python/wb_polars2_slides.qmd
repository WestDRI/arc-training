...
There are no downsides to using it, beside the effort of changing habits. For new users who don't have habits yet, there are just no downsides

Yet, most Python intro courses still teach the old tool
...
Let's play with data from the [GBIF](https://www.gbif.org/) website (free and open access biodiversity database)
...
<br>Run a query on it (no impact on memory, LazyFrame returned instantly):
...
<br>Depending on the subset returned by your query, the memory usage will vary. Here, the collected result is minuscule (79 bytes):
...
## How much RAM is actually needed? {.center}

I did a little experiment on a cluster:

I ran the previous code on a single CPU core and gradually reduced the amount of memory asked from Slurm until I got an OOM error
