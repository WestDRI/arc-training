...
Now, create a script for Slurm in the same fashion (let's call it `script.sh`):
...
We aren't mentioning any number of CPUs so Slurm will default to one CPU. We also aren't mentioning any memory, so Slurm will default to 256 MB per CPU.
