...
User settings saved in `~/.config/marimo/marimo.toml` or similar in other OS

Logs found at `~/.cache/marimo/logs` or similar in other OS
...
For more tutorials, replace `intro` with:
...
IPython magics replaced by Python functions

# Usage

## Package installation {.center}

User [settings]{.codelike} ➔ [Package Management]{.codelike} ➔ Manager: `uv`
