...
Let's start with the same DataFrame we used in the previous section:
...
If we want to write in place, we can reassign the output to the initial DataFrame:
...
There are [many ways](https://docs.pola.rs/user-guide/expressions/column-selections/) to select columns from a DataFrame.
...
Notice that the rows became out of order. Not to worry about order makes the code more efficient and does not affect future subsetting of our DataFrame. If you want to maintain the order however, you can use the `maintain_order` parameter:
...
<!-- To get a world total of number of cases for each day, we simply need to use the `sum` method on either the initial DataFrame or our  -->
