...
For more information on uv, you can have a look at [the webinar](wb_uv.qmd) I gave on this tool in May 2025.

:::

### Non-Python dependencies

While `uv` is fantastic to handle all things Python-related (Python versions, packages, environments, projects, etc.), if you need to install a software stack involving non-Python dependencies on your local machine, you will need to use [Pixi](https://github.com/prefix-dev/pixi), [Mamba](https://mamba.readthedocs.io/en/latest/), [conda](https://docs.conda.io/projects/conda/en/stable/), or another package manager that can handle non-Python packages (personally, I really like `pixi`: it is modern, written in Rust and thus very fast, it uses `uv`, it gives access to the same ecosystem of packages as `conda`, but does a much better job).
...
When you launch JupyterLab for the first time, a Launcher tab opens automatically (if you have a notebook open and the Launcher doesn't open automatically, you can open it by going to [File]{.codelike} in the top menu, then [New Launcher]{.codelike} from the drop-down).

In the Launcher, click on the button "Terminal" in the "Other" section and that will open a terminal in the environment of your notebooks. You can run the `pip` and `uv` commands in that terminal to install packages.
