...
```{python}
def add_substract_one(value):
    return value + 1, value - 1
```

You can define variables within functions. It makes things more readable when functions become complicated. In that case, you return the variables internal to the function that you will need in the global environment at the end of the function definition:

```{python}
def add_substract_one(value):
    plus_one = value + 1
    menus_one = value - 1
    return plus_one, menus_one
```
...
You get individual values by indexing:

```{python}
result = add_substract_one(4)

print(result[0])
print(result[1])
```

You can also assign the results to variables:

```{python}
added, substracted = result
