...
That's a speedup of almost 10 (the longer the series, the larger this speedup will be). This speedup is due to the fact that Polars is written in Rust and has parallel execution built-in. With Pandas, you would have to use [Dask](https://github.com/dask/dask) to achieve a similar behaviour.

Polars: 1, Pandas: 0 🙂

##### Lazy evaluation

Now, let's go back to [the jeopardy example with a large file](https://wgpages.netlify.app/python2/python-13-pandas/#example-with-a-larger-dataframe).

[Lazy evaluation is not yet implemented when reading files from the cloud](https://github.com/pola-rs/polars/issues/13115) (Polars is a very new tool and its functionalities are expanding fast).

This means that we cannot test the benefit of lazy evaluation in our example by using the CSV file in its current location (<https://github.com/pola-rs/polars/issues/13115>).

I downloaded it on my machine however to run the test.
...
```{.python}
df_pd = pd.read_csv("jeopardy.csv")
...
```{.python}
df_pl = pl.scan_csv("jeopardy.csv")
...
That's a speedup of 25 (the larger the file, the larger this speedup will be).
